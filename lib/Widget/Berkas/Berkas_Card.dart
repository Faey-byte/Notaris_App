...
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:intl/intl.dart'; // tambah dependency: intl
import 'package:notaris_app/Model/Ppat_Model.dart';
import 'package:notaris_app/Pages/detail_berkas_ppat.dart';

class BerkasCard extends StatelessWidget {
  final BerkasModel data; // ganti dynamic ke BerkasModel

  const BerkasCard({super.key, required this.data});

  _StatusStyle _getStatusStyle(String status) {
    switch (status.toLowerCase()) { // lowercase biar aman
      case "proses":
        return _StatusStyle(
            "PROSES BPN", const Color(0xFFFF9800), const Color(0xFFFFF3E0));
      case "selesai":
      ...
      case "pending":
        return _StatusStyle(
            "PENDING", const Color(0xFF2196F3), const Color(0xFFE3F2FD));
      default:
        return _StatusStyle(
          ...
          children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        data.client.publicID, // ← public ID
                        style: const TextStyle(
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    data.caseData.caseName.replaceAll('_', ' ').toUpperCase(), // ← nama kasus
                ...
                const SizedBox(width: 6),
                Text(
                  _formatTimestamp(data.createdAt), // ← format timestamp
                  style: const TextStyle(fontSize: 13, color: Color(0xFF555555)),
                ),
              ],
            ),
